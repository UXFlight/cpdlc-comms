...
export default function Message({message }: Props) {
  ...
  const handleClick = () => { //eventuellement devient un reducer
  ...
  return (
    <div className={`flex justify-center items-center ${message.state === "new" ? "cursor-pointer" : ""}`} onClick={() => handleClick()}>
      <img src={message.state === "new" ? "/up-arrow.svg" : "/arrow-up-bold-box.svg"} alt="arrow" className="w-[22px] h-[22px] mb-[30px]"></img>
      <div className="container flex flex-col items-start w-[516px] h-[87px] p-[8px]">
        <div className="flex items-center w-full h-auto justify-between align-baseline">
          ...
          <div className="h-[17px] bg-white-10 rounded flex items-center justify-center">
            <span className={`uppercase text-[14px] ${colorState()} font-semibold`}>{message.state}</span> 
          </div>
        </div>
        <div className="-mt-1 flex items-center w-full h-auto justify-between align-baseline">
          <span className={`font-semibold text-[16px] not-italic leading-norm ${colorState()} self-stretch tracking-wider`}>{message.element}</span>
