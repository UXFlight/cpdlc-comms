    ...
    >
      <div className={`flex flex-col gap-4 mt-3 ${isOpen ? "" : "hidden"}`}>
        <div className="flex items-center gap-4">
          <span className="text-white/80 text-sm min-w-[190px]">Offset distance and direction</span>
          <input
            type="text"
            value={offset}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-8">
            <CustomRadio label="Position" value="position" selected={step || ""} onChange={() => setStep("position")} />
            <CustomRadio label="Time" value="time" selected={step || ""} onChange={()=>setStep("time")} />
            <input
              type="text"
              value={stepValue}
